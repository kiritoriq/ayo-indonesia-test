var KTWizard3 = function () {
    ...
    var initWizard = function initWizard() {
        ...
        _wizard.on('beforeNext', function (wizard) {
            validator[wizard.getStep() - 1].validate().then(function (status) {
                //if (status == 'Valid') {
                    _wizard.goNext();
    ...
    var initValidation = function initValidation() {
        // Step 1
        validator.push(FormValidation.formValidation(_formEl, {
            fields: {
                klasifikasi: {
                    validators: {
                        notEmpty: {
    ...
    return {
        init: function init() {
            _wizardEl = KTUtil.getById('frm_wizard_surat_edaran');
            _formEl = KTUtil.getById('frm_surat_edaran');
            initWizard();
            initValidation();
        }
    };
...
$(document).ready(function () {
	...
	initUploadLampiran('lampiran_surat_edaran', '#add_lampiran_file')
	...
	$("#add_persetujuan").click(function(e){
		e.preventDefault();
		let form ='<tr id="a'+a+'">';
		form += '<td class="text-center"><a class="btn btn-sm btn-danger delete_app" onclick="delete_app(`'+a+'`)" key="'+a+'"><i class="flaticon-delete"></i></a></td>';
