...
<div class="container py-5 max-w-800px">
    <div class="card card-custom gutter-b">
        ...
        <form class="form" id="member_form" method="POST" action="{{ route('member.update', $member->id) }}">
            @method('PUT')
            @csrf
            <div class="card-body">
                <div class="form-group">
                    <label>Nama Anggota</label>
                    <input type="text" name="name" class="form-control" value="{{ $member->name }}" placeholder="Masukkan Nama Anggota">
                </div>
                <div class="form-group">
                    <label>Tinggi Badan</label>
                    <div class="input-group">
                        <input type="number" name="height" class="form-control" min="0" value="{{ $member->height }}" placeholder="Masukkan Tinggi Badan Anggota">
                ...
                <div class="form-group">
                    ...
                    <div class="input-group">
                        <input type="number" name="weight" class="form-control" min="0" value="{{ $member->weight }}" placeholder="Masukkan Berat Badan Anggota">
                        <div class="input-group-append">
                ...
                <div class="form-group">
                    ...
                    <input type="email" class="form-control" id="email" name="email" value="{{ $member->email }}" placeholder="Masukkan alamat email Anggota">
                </div>
                <div class="form-group">
                    <label>Nomor Ponsel</label>
                    <input type="text" class="form-control" id="phone_number" name="phone_number" value="{{ $member->phone_number }}" placeholder="Masukkan nomor ponsel Anggota">
                </div>
                <div class="form-group">
                    ...
                    <table id="org-table" class="table table-hover">
                        ...
                        <tbody id="org-result">
                            @if(count($member->org_member) > 0)
                                @foreach ($member->org_member as $index => $org_member)
                                    <tr id="row-org-{{ $index }}">
                                        <td>
                                            <select name="org[{{ $index }}][position_id]" id="position-{{ $index }}" class="form-control">
                                                <option value="1" {{ ($org_member->position_id == 1 ? 'selected' : '') }}>Ketua</option>
                                                <option value="2" {{ ($org_member->position_id == 2 ? 'selected' : '') }}>Staf</option>
                                                <option value="3" {{ ($org_member->position_id == 3 ? 'selected' : '') }}>Anggota</option>
                                            </select>
                                        </td>
                                        <td>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        let i = parseInt('{{ count($member->org_member) }}');
